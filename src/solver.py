...
class stokes2d:
    ...
    def compute_kernels(self):
        ...
        with np.errstate(divide='ignore',invalid='ignore'):
            ...
            K2 = -da_ * (-d/np.conjugate(dt) + np.conjugate(d)*dt/(np.conjugate(dt**2))) / (2j*np.pi)
    ...
    def compute_velocity(self,z):
        ...
        if isinstance(z,numbers.Number):
            ...
            phi = np.sum((omega*dt)[np.newaxis,:]/t_minus_z,axis=1)/(2j*np.pi)
            d_phi = np.sum((omega*dt)[np.newaxis,:]/(t_minus_z_sq),axis=1)/(2j*np.pi)

            psi = (1/(2j*np.pi))*(
                2*np.sum(np.real((np.conjugate(omega)*dt)[np.newaxis,:])/t_minus_z,axis=1)
    ...
    def compute_pressure(self,z):
        ...
        if isinstance(z,numbers.Number):    
            ...
            dd_phi = np.sum((omega*dt)[np.newaxis,:]/t_minus_z_cubic,axis=1)/(1j*np.pi)
