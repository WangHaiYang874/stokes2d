...
class MultiplyConnectedPipe:
    ...
    cs: ndarray             # shape=(n_interior_boundaries), dtype=complex128
    ...
    def build(self, max_distance=None, legendre_ratio=None, tol=None, n_jobs=1, fmm=True):
        if self.n_boundaries == 1:
            assert False
        assert fmm, "not implemented the preconditioning for direct"
    ...
    def build_A(self):
        ...
        assert False, "haven't implement the preconditioning yet for this. "
    ...
    def compute_omega(self, H, tol=None,max_iter=None):
        ...
        c = c_sep[:self.n_boundaries-1] + 1j*c_sep[self.n_boundaries-1:]
        return omega,c
    ...
    def build_omegas(self, tol=None, n_jobs=1):
        ...
        if n_jobs == 1:
            ...
            omegas_and_cs = array(Parallel(n_jobs=n_jobs)(
                delayed(lambda i: self.compute_omega(
                    self.boundary_value(i), tol))(i)
                for i in range(self.n_flows)))
        
        self.omegas = [omega for omega,_ in omegas_and_cs]
        self.cs = [c for _,c in omegas_and_cs]

    def build_pressure_drops(self):
        pts = array([let.matching_pt for let in self.lets])
        pressure_drops = []
        for omega,c in zip(self.omegas,self.cs):
            pressure = self.pressure(pts[:, 0], pts[:, 1], omega,c)
    ...
    def phi(self, z, omega,c,fmm=True):
        assert z.ndim == 1
        assert fmm

        if fmm: return self.fmm.phi(z.real, z.imag, omega,c)
    ...
    def d_phi(self, z, omega,c, fmm=True):
        assert z.ndim == 1
        assert fmm

        if fmm: return self.fmm.d_phi(z.real, z.imag, omega,c)
    ...
    def psi(self, z, omega, c, fmm=True):
        assert z.ndim == 1
        assert fmm
        
        if fmm: return self.fmm.psi(z.real, z.imag, omega,c)
    ...
    def velocity(self, x, y, omega,c, fmm=True):
        ...
        return H2U((self.phi(z, omega,c,fmm) + z * np.conj(self.d_phi(z, omega,c,fmm)) + np.conj(self.psi(z, omega,c,fmm))))

    def pressure_and_vorticity(self, x, y, omega,c, fmm=True):
        ...
        d_phi = self.d_phi(z, omega, c, fmm)
        d_phi_init = self.d_phi(
            array([self.lets[0].matching_pt[0]+1j*self.lets[0].matching_pt[1]]), omega,c,fmm)
    ...
    def pressure(self, x, y, omega,c, fmm=True):
        assert fmm
        return self.pressure_and_vorticity(x, y, omega,c,fmm)[0]
    ...
    def build_plotting_data(self, xs, ys, interior,fmm=True):
        ...
        for omega,c in zip(self.omegas,self.cs):
            ...
            velocity = self.velocity(xs[interior], ys[interior], omega,c, fmm)
            pressure, vorticity = \
                self.pressure_and_vorticity(xs[interior], ys[interior], omega,c, fmm)
            base_pressure = self.pressure_and_vorticity(base_x, base_y, omega,c, fmm)[0][0]
    ...
    def build_pressure_drops(self):
        ...
        pressure_drops = []
        for omega,c in zip(self.omegas,self.cs):
            pressure = self.pressure(pts[:, 0], pts[:, 1], omega,c,fmm=True)
            pressure_drops.append(pressure[1:] - pressure[0])
