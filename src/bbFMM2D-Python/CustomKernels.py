...
from numpy import indices, sqrt, empty, meshgrid, conjugate
import math
...
class CustomKernels:
    ...
    @staticmethod
    def phi_integrand(z, zeta):
        return 1/(zeta-z)
    
    @staticmethod 
    def d_phi_integrand(z, zeta):
        return -1/(zeta-z)**2
    
    @staticmethod
    def psi_integrand1(z,zeta):
        return 1/(zeta-z)
    
    @staticmethod
    def psi_integrand2(z,zeta):
        return -conjugate(zeta)/(zeta-z)**2
    ...
    def exampleKernelC(M, x, N, y):
        ...
        # this made it clear that x represents the source points and y the target points. 
        kernel = empty([M, N])
        for i in range(0, M):
            for j in range(0, N):
